...
class subnet{
  ...
  public static void main(String args[]) {
    ...
    for (int i = 0; i < 4; i++) {
      split_bip[i] =
        appendZeroes(Integer.toBinaryString(Integer.parseInt(split_ip[i])));
      bip += split_bip[i];
    }
    ...
    for (int j = 1; j < scount; j++) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        } else System.out.println(ipAddr[i] = ipAddr[i] + total_address);
      }
      System.out.println(" GROUP " + (j + 1) + " LAST ADDRESS:");
      for (int i = 0; i < 4; i++) {
        if (i < 3) {
          System.out.print(ipLast[i] + ".");
        } else System.out.println(ipLast[i] = ipLast[i] + total_address);
      }
      System.out.println();
    }
    try {
